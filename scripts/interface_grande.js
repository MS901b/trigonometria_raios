...
var Tabela = Class.create({
	...
	initialize: function(data, questao) 
	{
		...
		for(var a = 0; a <= data[0].length; a++)
		{
			if(a == data[0].length)
			{
				tmp = new Element('th', {className: 'sem_borda'}).update('');
				tr.insert(tmp);
...
var Multiplo = Class.create({
	...
	podeEsconder: function(ev)
	{
		...
		window.setTimeout(this.sePuderEsconde.bind({ajuda: ajuda, input: input}), 100);
	},
	sePuderEsconde: function()
	{
		if(!this.ajuda.hasClassName('com_focus') && !this.input.hasClassName('com_focus'))
			this.ajuda.hide();
	}
});
...
function gerencia_partes()
{
	...
	if(nomeSoft == null)
		alerta('A variável nomeSoft não está definida.');
	if(PosicaoAtual.Atividade == null)
		alerta('A atividade atual deve ser especificada na variável PosicaoAtual.Atividade.');
